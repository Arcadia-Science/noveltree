...
tree <- unroot(read.tree(asteroid_tree))

# Reroot the tree
if(length(outgroups) > 1){
    tree$node.label <- 1:tree$Nnode
    mrca <- getMRCA(tree, outgroups)
    tree <- reroot(tree, node = mrca, position = 0.5)
} else {
    tree$node.label <- 1:tree$Nnode
    parent_node <- 
        tree$edge[,1][which(tree$edge[,2] == match(outgroups, tree$tip.label))]
    tree <- reroot(tree, node = parent_node, position = 0.5)
}

# Write the rerooted tree to a file
write.tree(tree, file = "asteroid_rooted.bestTree.newick")